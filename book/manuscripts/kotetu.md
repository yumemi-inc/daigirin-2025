...
エッジ AI のメリットとしては、サーバーへの情報送信を必要とせず端末内で AI 処理が完結することから、セキュリティ面でのメリットやリアルタイム処理に適していることが挙げられます。特にセキュリティ面については、クラウド AI 系のサービスは送信した情報がどのように使われているのかが見えにくい点や情報漏洩の点でリスクがありますが、エッジ AI であればそもそもサーバーへデータを送信する必要が無いためより安全に AI 機能を利用できるメリットがあります。

一方、エッジ AI のデメリットとしては、エッジデバイスの性能がボトルネックとなり、搭載できる AI 処理に制約が生じるや、エッジ AI で使用する学習済みモデルの漏洩リスクが挙げられます。ChatGPT といった生成 AI 系のサービスは、一般的には Web サービスといったクラウド AI として提供されます。Web サービスにすることで、生成 AI の学習・推論処理に必要な大量のコンピュータリソースを確保できるだけでなく、モデルの漏洩リスクを防げるメリットがあります。逆に、 ChatGPT と同様の大量のデータを利用した推論処理をサーバーに比べて性能が大幅に劣るエッジデバイスで実行することは現実的ではありません。

エッジデバイスの制約により、エッジ AI では実現が難しいことはまだ多く存在します。一方で、エッジデバイスの性能は日々進化しており、特に GPU や Neural Engine（ AI 処理に特化したプロセッサユニット）の進化は目覚ましいものがあります。今後の性能向上により、より多くの AI 処理がエッジ AI で実現できるようになっていくのではないでしょうか。
