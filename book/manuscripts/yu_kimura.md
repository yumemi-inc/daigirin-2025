...
私が参加するチーム KatLab は、アドバンストクラスに参加しました。
...
各コースにつき 1 回、計 2 回の走行で獲得したポイントを比較し、高いポイントの方を走行部門の成績として採用します。

![コースの全景](./images_yu_kimura/course.png 'コースの全景')

走行部門では、ポイント獲得対象として、走行ポイントとボーナスポイントの 2 種類のポイントを獲得できます。
...
その反射光をカラーセンサーが読み取ることで、反射光の RGB 値を取得し、色を判定しています。
...
1. 録画を停止する
1. 録画した動画から、計測したバウンディングボックス侵入・退出タイミングから中間のフレームを切り出す
...
これにより、ET ロボコン全体、走行体内部のそれぞれの間での関係性を示すことで、**開発対象**と**ソフトウェアの開発対象**を、コンテキスト分析でそれぞれ示しました。
...
ソフトウェア構造から、各難所攻略の流れを表す状態遷移図を示しました。
状態遷移図内のそれぞれの状態に着目し、主要なユースケースが実現可能であることを、コミュニケーション図に示しました。
...
2 つの円の難所を攻略するという状態から、真円のエリア到達時に、プラレール・背景撮影攻略中という状態へ遷移することを、状態遷移図を示しました。
...
具体的には、大会の成績についてと、2024 年度のチームリーダーとして得た経験についてを、それぞれ書きます。
最後に、それらについての感想と、来年度の目標について書きます。

### 大会成績

2024 年度は、九州地区大会で走行部門 2 位、モデル部門 1 位、総合 2 位。
チャンピオンシップ大会（全国大会）で走行部門、モデル部門、総合 5 位の成績を収めました。
KatLab は 2018 年から 7 年連続で全国出場を果たしていたため、その記録を継続させることができました。
企業も参加した中で、学生団体では、総合の成績は上から 2 番目でした。
それを考慮すると、入賞を逃しましたが、良い成績を収められたと考えています。
...
だからこそ、システム設計書でソフトウェアの構造や振舞いを理解することや、設計そのものの良し悪しを判断することの重要さを感じていただけたのではないでしょうか。
