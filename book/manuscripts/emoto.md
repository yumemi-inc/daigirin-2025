...
<div class="doc-header">
  <h1>プログラムのパフォーマンス計測と最適化</h1>
  <div class="doc-author">江本光晴</div>
</div>

# プログラムのパフォーマンス計測と最適化

プログラムのパフォーマンスは、アプリケーションの動作速度やユーザー体験に大きく影響を与えます。しかし、単に「速い」コードを書くことだけが重要なのではなく、適切な計測を行い、ボトルネックを特定し、効率的な最適化を行うことが求められます。

本書では、SwiftとPythonを用いたパフォーマンス計測手法を詳細に解説し、計測データの分析から最適化までの流れを示します。単なる理論的な説明にとどまらず、実際のコード例を多数掲載し、実践的なアプローチで学ぶことを目指します。

## パフォーマンス計測の基礎

プログラムのパフォーマンスを向上させるためには、まず現状の計測を正しく行うことが必要です。計測なしに最適化を行うと、改善の方向性が分からず、逆にパフォーマンスが低下する可能性があります。そのため、計測を適切に実施し、改善点を明確にすることが重要です。

### 計測の基本指標

プログラムの実行速度を測定する際、考慮すべき主要な指標は次の通りです。

| 指標 | 説明 |
| :-- | :-- |
| 実行時間 | コードが完了するまでの時間（秒またはミリ秒単位で計測） |
| CPU使用率 | 処理にどの程度のCPUリソースを使用したか |
| メモリ使用量 | プログラムが消費するメモリのサイズ |
...
CPU使用率(%)は、プログラムがどの程度のCPU時間を使用しているかを示す指標であり、次の方法で計算されます。

```math
{\rm CPU}使用率 = \frac{(ユーザー時間+システム時間)}{経過時間}
```

例えば、プログラムが 2 秒間 CPU を使用し、総実行時間が 10 秒の場合、CPU使用率は 20% となります。


## Swift、Python、GNU time を使ったパフォーマンス計測

// TODO: 動作確認

プログラムのパフォーマンスを測定する方法はいくつかありますが、本章では Swift、Python、および GNU time コマンドを使用して計測する方法を紹介します。

```shell
% ./measure -e {計測対象の実行ファイルのパス} -c {試行回数}
```

### Swift による計測

Swift でコマンドラインで引数を取得する
